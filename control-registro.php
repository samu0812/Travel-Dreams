...
$fecha_nacimiento = $direccion = $telefono = $email = $nombre = $apellido = $dni = $password = "";
    ...
    if (!empty($_POST['fecha_nacimiento']) && !empty($_POST['direccion']) && !empty($_POST['telefono']) && !empty($_POST['correo']) && !empty($_POST['nombre']) && !empty($_POST['apellido']) && !empty($_POST['dni']) && !empty($_POST['password'])) {
        ...
        if (isset($_POST["registro"])) {
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                ?>
                <h3 class="bad">¡El usuario ya existe en la base de datos!</h3>
                <?php
            } else {
        ...
        // Alguno de los campos está vacío, mostrar un mensaje de error
        ?>
